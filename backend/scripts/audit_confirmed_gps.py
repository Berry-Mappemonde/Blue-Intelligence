"""GPS audit of confirmed seeds in Atlas. No seeds/build.

python3 scripts/audit_confirmed_gps.py              # dry-run (default)
python3 scripts/audit_confirmed_gps.py --dry-run
python3 scripts/audit_confirmed_gps.py --persist    # correct obvious cases
"""
...
from app.config import DB_NAME  # noqa: F401  — load the .env
...
def main() -> None:
    ...
    p.add_argument("--dry-run", action="store_true", default=False,
                   help="Print the report without writing (default if no --persist)")
    p.add_argument("--persist", action="store_true",
                   help="Write gps_audit_* and correct obvious GPS")
    args = p.parse_args()

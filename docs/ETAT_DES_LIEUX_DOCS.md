...
Version **1.1** — 22 septembre 2026 (lot D0). L’index est
[`docs/README.md`](README.md). Les plans **HIST** sont dans
[`archives/`](archives/) (bandeau en tête). Version **1.0** : 20 septembre
2026 (soir, première nuit de lots).
...
**n'est pas implémenté** (la mémoire à ne pas perdre).
...
| `README.md` (index) · `MANUEL_UTILISATEUR.md` · `USER_MANUAL.md` | **RÉF** | lot D0 (22 sept.) : index, manuels FR/EN | tenir à jour quand un bouton change |
| `LOTS_ORDRE_ET_PROMPTS.md` | **PLAN** | 33 lots, prompts ; P2, L1, M, N, O, F1, F2, L2, F3, F4, L3 exécutés le 20 sept. | tenir la colonne « fait » ; lots W et D0 ajoutés |
...
| `archives/PLAN_GENERAL_DEVELOPPEMENT_SIMULATEUR.md` v2 | **HIST** | réanalyse de chaque idée (18 sept.), décisions du porteur § 5, lots A–K faits | § 4 « ce qu'on ne fait pas » à relire : ordre vocal / dictée du journal, page carnet de bord (1.5) **non faits** ; LangSmith écarté ; sécurité P0 = secret admin partagé (compte utilisateur « plus tard ») |
| `archives/PLAN_LOTS_AGENT_SIMULATEUR.md` v1.1 | **HIST** | lots P → K tous mergés (#189–#202) | archivé le 22 sept. (bandeau « réalisé ») |
| `archives/PLAN_CHANTIERS_STRUCTURANTS_SIMULATEUR.md` | **HIST** (16 l.) | pointeur vers le pipeline d'affichage | archivé ; voir `archives/PLAN_PIPELINE_AFFICHAGE_SIMULATEUR.md` |
| `archives/PLAN_PIPELINE_AFFICHAGE_SIMULATEUR.md` | **HIST/mémoire** | P1 GRIB `pending` fait ; profil prod fait (lot J) | **non faits, à garder en mémoire** : catalogues spatiaux complets (tuiles vectorielles, clustering) si les données grossissent ; pipeline météo asynchrone partagé « tous fournisseurs » (statut durable + cache par cycle) ; Web Worker (conditionné au profil : pas nécessaire aujourd'hui) ; `useWakeLayer` tableaux complets à chaque tick |
| `archives/PLAN_UI_PRODUIT_SIMULATEUR.md` | **HIST** | lots UI 1–9 faits | lot 10 « Recalculer l'itinéraire » **volontairement hors périmètre** (le bouton public reste ; G3 conseil de route en Suivre non fait) |
| `archives/PLAN_IMPLEMENTATION_SKIPPER_VIRTUEL.md` v1.1 | **HIST** | S1 → S5 mergés ; S6 (contraintes recompute) et S7 faits dans les lots G/K | « S6+ : Confort (3 pills) » **non fait** ; ordre vocal non fait |
| `archives/PLAN_IMPLEMENTATION_EVENEMENTS_ICI_LLM.md` v1.5 | **HIST** | juge NOW/FREE, dossier `ici()`, récit LLM, StoryJob, pré-génération (lots E1–E5 puis P/F) | E5 « file récit / extract côté serveur » : partiellement (story_cache) ; NVIDIA / Tavily → L1–L6 |
| `archives/PLAN_IMPLEMENTATION_SIMULATOR_SUIVRE_ET_SIMULATION.md` v2 | **HIST** | les deux modes existent | archivé le 22 sept. |
| `archives/PLAN_IMPLEMENTATION_SIMULATION_A.md` / `_B.md` | **HIST** | horloge climatologique (A) faite ; bateau virtuel (B) fait (recompute, `spliced`) | B « English : not yet » → i18n fait depuis |
| `archives/PLAN_IMPLEMENTATION_NAVIGUIDE_SIMULATOR_ETAPE1.md` (+ `.en`) | **HIST** | étape 1 livrée (13 sept.) | archivé (1 200 lignes ×2) |
| `archives/PLAN_IMPLEMENTATION_CLIMATOLOGIE.md` | **HIST/CDC BI** | mode climatologie BI (7ᵉ mode) livré ; atlas consommé par le simulateur | « **swell not shipped** » (houle climatologique non livrée) — mémoire |
| `archives/PLAN_IMPLEMENTATION_FILIERES_CARTO.md` + `audits/2026-09-14-filieres-diagnostic.md` | **HIST/CDC BI** | filières contrôle/carte/hydro/satellite | « catalogue `candidat` (feux, roches, mouillages) pas encore dans un pipeline » — mémoire |
| `audits/PROFIL_BUILD_PROD_2026-09-19.md` | **AUDIT** | profil prod (lot J) : React ≈ 2,5 s CPU / 60 s | refaire après U (zoom) ; sert de méthode |
...
| `archives/hackathon-nebius-nvidia.md` (+ `.en`) | **HIST** | cahier unique du 13–14 sept. (avant les lots) | § 8 périmé ; crédits § 7 utiles ; **remplacé** par `PLAN_NEMOTRON_NEBIUS_TAVILY.md` + `HACKATHON_DEVPOST_SOUMISSION.md` |
| `archives/HACKATHON_OPEN_AGENT_2026.md` | **HIST** | autre hackathon (synthèse) | archivé le 22 sept. |
| `archives/nvidia-llm-audit.md` | **HIST** | replis NIM par usage (avant Token Factory) | remplacé par la cascade L1 |
| `poe-*.md` (3 fichiers) + `data/*.json` | **AUDIT BI** | dossiers d'audit PoE (GPS, seeds, name_only) | **laissés en place** : le backend cite ces chemins (`run_rules.json`, services PoE) |
...
## 3. Rangement (lot D0, 22 septembre 2026)

**Fait.** Index [`README.md`](README.md). Manuels
[`MANUEL_UTILISATEUR.md`](MANUEL_UTILISATEUR.md) /
[`USER_MANUAL.md`](USER_MANUAL.md). Plans **HIST** déplacés dans
[`archives/`](archives/) avec bandeau. `PROFIL_BUILD_PROD_2026-09-19.md`
déplacé dans [`audits/`](audits/). Liens des documents vivants réécrits.
`.cursor/rules` : aucun chemin HIST. Script
[`check_md_links.py`](check_md_links.py).

**Décisions (notées).** (1) Les audits `poe-*.md` et `data/` restent à
`docs/` : le backend les cite ; les déplacer casserait le code.
(2) Le découpage `plans/` · `hackathon/` · `cdc/` n’est **pas** fait :
les prompts des agents citent encore `docs/PLAN_*.md`.
(3) `ARCHITECTURE.md` n’est **pas** réécrit (hors liste de fichiers du lot).

Arbre cible **restant** (après H1 ou un lot docs suivant) :

```
docs/
  README.md  REGLES_WORKFLOW_AGENT.md  ESPRIT_DE_L_APPLICATION.md
  MANUEL_UTILISATEUR.md  USER_MANUAL.md  REGLES_PARAMETRES.md
  plans/     LOTS_ORDRE_ET_PROMPTS, PLAN_LOTS_COMPLEMENTAIRE, PLAN_FILM,
             PLAN_NEMOTRON, PLAN_AUDIT_CALCULS, PLAN_GLOBE_3D, WORKFLOW_INDUSTRIEL
  hackathon/ HACKATHON_DEVPOST_SOUMISSION
  cdc/       CAHIER_DES_CHARGES_*, CONTRATS_*, PRD, CATALOGUE_SEAMARK, ici-tout
  audits/    déjà en place (PROFIL_BUILD_PROD y est)
  recette/   captures par lot
  archives/  déjà en place (plans HIST)
```
